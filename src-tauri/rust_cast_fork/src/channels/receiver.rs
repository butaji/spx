    ...
    W: Write + Read,
{
    ...
    /// Sends a message to a specific destination on a custom namespace.
    ///
    /// This is needed for applications such as Spotify that listen on a
    /// transport-specific destination rather than the broadcast destination.
    pub fn send_message<M: Serialize, S>(&self, namespace: &str, destination: S, message: &M) -> Result<(), Error>
    where
        S: Into<Cow<'a, str>>,
    {
        if !namespace.starts_with("urn:x-cast:") {
